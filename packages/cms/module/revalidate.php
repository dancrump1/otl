...
class Revalidate extends BaseModule
{
    public function init()
    {
        ...
        Event::on(Node::class, Node::EVENT_AFTER_SAVE, function (ModelEvent $event) {
            // Only trigger if we're in the control panel and in the navigation section
            $request = Craft::$app->getRequest();
            if ($request->getIsCpRequest()) {
                $segments = $request->getSegments();
                // Check if we're in the navigation section (e.g., /admin/navigation/...)
                if (!empty($segments) && isset($segments[0]) && $segments[0] === 'navigation') {
...
class RevalidateJob extends BaseJob
{
    ...
    public function execute($queue): void
    {
        ...
        $client = new Client();
        $urlBase = App::env('UI_BASE_URL') . '/api/revalidate?secret=' . App::env('REVALIDATION_SECRET');
