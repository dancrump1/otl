...
import { getBrickHealth } from './levels'
import {
 detectCollision,
...
function getBrickPlayArea(
 config: BrickBreakerConfig,
 dimensions: CanvasDimensions
) {
 const { topPadding, sidePadding, playAreaHeight: playAreaRatio, brickFieldFit, brickFieldAspect } =
  config.layout
 const { width, height } = dimensions
 const { paddleOffset, paddleHeight } = config.sizing

 if (brickFieldFit === 'contain') {
  const top = height * topPadding
  const bottom =
   height * (1 - paddleOffset) - height * paddleHeight - Math.max(24, height * 0.04)
  const availableW = width * (1 - 2 * sidePadding)
  const availableH = Math.max(bottom - top, 1)
  const aspect = brickFieldAspect > 0 ? brickFieldAspect : 1
  ...
  return {
   x: (width - fieldW) / 2,
   y: top + Math.max(0, (availableH - fieldH) / 2),
   width: fieldW,
   height: fieldH,
  }
 }
 ...
 return {
  x: width * sidePadding,
  y: height * topPadding,
  width: width * (1 - 2 * sidePadding),
  height: height * playAreaRatio,
 }
}
...
function createBricksFromLevel(
 ...
 const { brickGap } = config.layout
 const rows = level.bricks.length
 const cols = level.bricks[0]?.length || config.layout.cols
 const playArea = getBrickPlayArea(config, dimensions)

 const brickWidth = (playArea.width - (cols - 1) * brickGap) / cols
 const brickHeight = (playArea.height - (rows - 1) * brickGap) / rows
 ...
 col,
 bounds: {
 x: playArea.x + col * (brickWidth + brickGap),
 y: playArea.y + row * (brickHeight + brickGap),
 width: brickWidth,
 height: brickHeight,
 },
 type: def.type,
